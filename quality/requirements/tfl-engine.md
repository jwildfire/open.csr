...
| TFL-PREP-008 | Every vendored PHUSE file — eleven ADaM datasets and the study's SDTM CM and DM domains — matches the SHA-256 and byte count recorded in `PROVENANCE.json`, which pins the upstream repository, commit, per-file git blob SHA-1 and MIT licence. | Traceability | `test-data-phuse.R` | Verified |
...
| TFL-PREP-018 | The medication analysis dataset is derived from the study's own SDTM CM domain — every CM column as published, dates taken only where stated to the day, the safety flag and treatment columns joined from ADSL, `CMFL` on every record — and reproduces every one of the 414 statistics the medication table published from the relabelled PHUSE copy, which stays vendored for that check. | Traceability | `test-data-phuse.R` | Verified |
| TFL-PREP-019 | DM, the study's SDTM demographics domain, is the screened population: `prepare_data()` keeps all 306 rows, including the 52 screen failures that never reached ADSL, and is the one dataset it does not restrict to ADSL's subjects. | Correctness | `test-data-phuse.R` | Verified |
