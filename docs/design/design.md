...
| D12 | **Demo data = CDISCPILOT01, from two packagings, chosen per dataset** (`data_sources()`). `{pharmaverseadam}` serves the five domains it always served — ADSL, ADAE, ADEX, ADLB, ADVS — and remains the only source of ADEX. The CDISC pilot submission's own ADaM package, vendored from `phuse-org/phuse-scripts` (MIT), serves the efficacy (ADQSADAS, ADQSCIBC, ADQSNPIX), laboratory (ADLBC, ADLBH, ADLBHY) and time-to-event (ADTTE) domains, plus ADCM. The tested `data-prep` layer still derives what a source leaves missing, and now derives *less* on the lane where the study states more. | Correction, 2026-08-27 (#39): the original decision read "no CDISCPILOT01 efficacy ADaM exists". That was true of `{pharmaverseadam}` and false in general — the pilot's own package contains both primary efficacy endpoints, ADAS-Cog (11) and CIBIC+, and its data guide names them as such. The rest of D12 stands: the correct answer to a gap is a tested, documented derivation layer, not oncology-shaped substitute data. The two packagings do not agree on every figure, so the default registry moves nothing the committed displays already stand on; the divergences are measured in `quality/data/source-agreement.json` and reproduced by `qc/source-agreement.R`.  **Revised 2026-09-02 (D0032 R2, #60):** the default is now the pilot's own package for every dataset it publishes; `{pharmaverseadam}` serves only ADEX and ADLB, which no display reads, and is the measured alternate. The "every domain pharmaverse already served stays there" rule was protecting six displays from change and turned out to be protecting the document from agreeing with itself — its actual-treatment column moves twelve subjects, so the report told two versions of the study. The last technical reason for it, ADEX, did not hold: the reference exposure table reads ADSL. |
...
| pharmaverseadam efficacy gap | Closed as a *data* gap in v0.3.0 (#39): the CDISC pilot's own ADQSADAS/ADQSCIBC/ADQSNPIX are vendored and preparable. Still open as a *display* gap — no efficacy display is specified, and the study's own analysis definitions (define.xml analysis results metadata) are the reference for specifying one |
