...
Written 2026-07-25, at the end of the kickoff session. v0 is live: [demo site](https://jwildfire.github.io/open.csr/), [research](../research/README.md), [design](design/design.md), [contracts](design/contracts.md).

## What v0 proved

The closed loop works end to end on real data. A change request to the AE table became a spec edit, regenerated as `v002` with a recorded actor and rationale, and the site renders the two-iteration history. Every number in the assembled CSR is a binding, and clicking one shows the dataset, the spec, the ARD file and its hash, the display, and the iteration that produced it. 70 testthat + 171 vitest tests, 188 reviewed requirements, 99% requirement coverage, published as evidence pages.
...
2. **Approve or reject the four draft text blocks** (`TXT-E3-1206`, `-1222`, `-1231`, `-1300`). They are agent-drafted prose in the `generated` tier, currently excluded from the assembled report by the approval gate. Approving them is the first real exercise of the human-in-the-loop contract — and reading them is the fastest way to judge whether agent-drafted CSR prose is any good.
...
- **A writing surface.** Editing markdown with `{{ard:...}}` bindings by hand is a programmer's workflow. The demo needs a page where a writer sees the prose, the displays it references, and a way to request a change — even if the change lands as a PR rather than a live edit.
...
- **Live request on the deployed site** (the v1 vision): request → dispatch → PR → preview. The audit-page pattern from hub#109 is directly reusable.
