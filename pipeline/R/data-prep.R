...
#'   \item{`TRT01A` (non-ADSL)}{Taken from the prepared ADSL by `USUBJID`, not
#'     from the dataset's own `TRTA`/`TRTP`, so treatment assignment is
#'     single-sourced across every display. Verified to be a no-op within each
#'     source.}
...
#' @noRd
attach_trt01a <- function(df, adsl) {
