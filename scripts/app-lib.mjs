...
export const APP_TABS = [
  // The bar reads as the pipeline (#82): what people write, what turns it into
  // outputs, what the pipeline writes. Templates describe what a report of this
  // kind IS, so they belong to no one study and stay in the header.
  ...
  { id: 'pipeline', label: 'Pipeline', href: '../pipeline/index.html', scope: 'study' },
...
export function buildNavTree({
  ...
  return {
    ...
    groups: [
      ...
      // What was measured (#76): one item per dataset, the id as its number
      // because that is how a display header and an ARD envelope name it.
      {
        id: 'data',
        label: 'Data',
      ...
      // What turns inputs into outputs (#82): one item per pipeline function.
      {
        id: 'pipeline',
        label: 'Pipeline',
        note: 'What turns inputs into outputs',
        items: pipeline.map((fn) => ({
          id: fn.id,
          label: fn.title || fn.id,
          number: fn.number || null,
          status: fn.status || 'ok'
        }))
      }
    ],
    // The explorer's three parts (#82), in the pipeline's order. `groups` above
    // stays the flat list every consumer already reads; the parts say how the
    // sidebar arranges them.
    parts: [
      { id: 'inputs', label: 'Inputs', note: 'what people write and the pipeline reads', groups: ['data', 'metadata', 'text'] },
      { id: 'pipeline', label: 'Pipeline', note: 'what turns inputs into outputs', groups: ['pipeline'] },
      { id: 'outputs', label: 'Outputs', note: 'what the pipeline writes and nobody edits', groups: ['displays', 'values', 'documents'] }
    ]
  };
}
...
const ITEM_KEY = { documents: 'doc', displays: 'display', text: 'block', values: 'focus', data: 'focus', metadata: 'focus', pipeline: 'focus' };
...
export function renderSidebar({ tree, active = null, selected = {} } = {}) {
  ...
  const groups = Array.isArray(tree.parts) && tree.parts.length
    ? tree.parts
        .map(
          (part) =>
            `<li class="nav-part" data-nav-part="${escapeHtml(part.id)}">` +
            `<div class="nav-part-head"><span class="nav-part-label">${escapeHtml(part.label)}</span>` +
            (part.note ? `<span class="nav-part-note">${escapeHtml(part.note)}</span>` : '') +
            `</div><ul class="nav-part-groups">` +
            part.groups.map((id) => byId.get(id)).filter(Boolean).map(renderGroup).join('') +
            `</ul></li>`
        )
        .join('')
    : tree.groups.map(renderGroup).join('');
  return (
...
export function renderValuesPane({
  ...
  root = '../',
  // For the flow's output link (#83): the repository the store is committed in.
  config = null
} = {}) {
...
/** The flow a value was made by (#83): one ARD row or its input values, through regenerate_values(), into its entry in the store. */
function valueFlow(value, { root = '../', config = null } = {}) {
  const inputs =
    value.kind === 'derived'
      ? (value.derivation?.inputs || []).map((id) => ({ label: id, sub: 'named value', href: `${root}values/index.html#${id}`, kind: 'data' }))
      : [{ label: value.source?.display || 'ARD row', sub: value.source?.address || '', href: value.source?.display ? `${root}gallery/${value.source.display}.html` : null, kind: 'data' }];
  const steps = [
    {
      label: 'regenerate_values()',
      sub: value.kind === 'derived' ? `${value.derivation?.op || 'derived'}(…)` : 'resolve the address',
      href: `${root}pipeline/regenerate-values.html`,
      kind: 'fn'
    }
  ];
  const outputs = [{ label: 'values.json', sub: `${value.id} = ${value.formatted ?? value.value ?? ''}`, href: repoBlob(config, 'outputs/values/values.json'), kind: 'out' }];
  return `<details class="flow-details"><summary>Flow</summary>${renderFlow({ label: `How ${value.id} was made`, inputs, steps, outputs, compact: true })}</details>`;
}
