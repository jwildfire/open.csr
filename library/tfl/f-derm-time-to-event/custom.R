...
#'   variable, `spec$param` the PARAMCD to analyse, and `spec$risk_times` the
#'   grid at which subjects at risk are counted for the strip under the curve.
...
km_time_to_event <- function(data, spec, denom) {
  ...
  risk_times <- as.numeric(spec$risk_times %||% numeric(0))
  ...
  for (lv in levels) {
    ...
    # The numbers-at-risk strip prints committed statistics, not a second
    # calculation: the count at each declared time is the size of the risk set,
    # the subjects whose observed time has not yet passed. It travels in the ARD
    # so a reader can check the strip against `ard.json` rather than against the
    # renderer's arithmetic.
    risk_n <- vapply(risk_times, function(t) sum(df$.time[keep] >= t), numeric(1))
    ...
    pieces <- c(pieces, list(
      ...
      row("risk_time", "Times at which subjects at risk are counted (days)", risk_times, "survival"),
      row("risk_n", "Subjects at risk", risk_n, "survival"),
